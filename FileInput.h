...
void FileInput(FILE *fp, FILE *rep_ptr);
	...
	//{
		...
		if (nvar > 0) 
		{
			...
			for(i = 0; i < nvar/2; i++)
			{
				...
				lim_r[i][0] = value[10+2*i];
				lim_r[i][1] = value[11+2*i];
			}
